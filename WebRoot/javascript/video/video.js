...
function initVideo(){
    ...
    $("#endTimes").val(szCurTime + " 23:59:59");
}

function cbSelWnd (xmlDoc) {
...
// 登录
function clickLogin(ip, port, user, pwd) {
    var szIP = ip,
        szPort = port,
        szUsername = user,
        szPassword = pwd;
...
function clickStartRealPlay2(iStreamType,num,iChannel) {
    g_iWndIndex = num;
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex);
    var szDeviceIdentify = $("#ip").val();
    iRtspPort = parseInt($("#rtspport").val(), 10);
    //iChannelID = parseInt($("#channels option:nth-child(" + (num + 1) + ")").val(), 10);
    iChannelID = iChannel;
    var bZeroChannel = $("#channels option:nth-child(" + (num + 1) + ")").attr("bZero") == "true" ? true : false,
        szInfo = "";
    ...
    var iRet = WebVideoCtrl.I_StartRealPlay(szDeviceIdentify, {
...
function clickCapturePic() {
    ...
    if (oWndInfo != null) {
        ...
        if (0 == iRet) {
            var objt = WebVideoCtrl.I_GetLocalCfg();
            var capturePath = $(objt).find("CapturePath").eq(0).text();
            szInfo = "抓图成功！"+(capturePath!=""?("抓图保存路径："+capturePath):"");
        } else {
            szInfo = "抓图失败！";
        }
        showOPInfo(oWndInfo.szIP + " " + szInfo);
...
function clickStartRecord() {
    ...
    if (oWndInfo != null) {
        ...
        if (0 == iRet) {
            var objt = WebVideoCtrl.I_GetLocalCfg();
            var recordPath = $(objt).find("RecordPath").eq(0).text();
            szInfo = "开始录像成功！"+(recordPath!=""?("录像文件保存路径："+recordPath):"");
        } else {
            szInfo = "开始录像失败！";
        }
        showOPInfo(oWndInfo.szIP + " " + szInfo);
        ...
        success: function (xmlDoc) {
            if ("MORE" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
                ...
                for (var i = 0, nLen = $(xmlDoc).find("searchMatchItem").length; i < nLen; i++) {
                    ...
                    var szEndTime = $(xmlDoc).find("endTimes").eq(i).text();
                    ...
                    objTd = objTr.insertCell(4);
                    objTd.width = "10%";
                    objTd.innerHTML = "<a href='javascript:;' onclick='clickStartDownloadRecord(" + i + ");' title='下载'><img src='./../javascript/jquery-easyui-1.4.4/themes/icons/download.png'></a>";
...
function clickStartDownloadRecord(i) {
    ...
    if (iDownloadID < 0) {
        ...
        $("<div id='downProcess' class='freeze' style='z-index:999999'></div>").appendTo("body");
...
function downProcess() {
    ...
    if (0 == iStatus) {
        ...
        if (iProcess < 0) {
            ...
            var DownloadPath = $(xmlDoc).find("DownloadPath").eq(0).text();
            var info = "录像下载完成!";
            if(DownloadPath!="" && DownloadPath!=undefined){
                info = info+"文件保存路径："+DownloadPath;
            }
            showOPInfo(info);
            clearInterval(tDownloadProcess);
            tDownloadProcess = 0;
